...
# Enhanced cleanup with comprehensive system cleanup for RPM-based distributions
...
log_warning() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [RPM-POSTREMOVE] ⚠️  $1" | tee -a "$LOG_FILE"
}
...
log "Starting DropSentinel comprehensive RPM post-removal cleanup..."
...
# Remove SELinux context (RPM-specific)
if command -v semanage >/dev/null 2>&1; then
    log "Removing SELinux context..."
    semanage fcontext -d "$INSTALL_DIR/$EXECUTABLE_NAME" 2>/dev/null || log_warning "Could not remove SELinux context"
fi
...
log_success "DropSentinel RPM post-removal cleanup completed"
exit 0
