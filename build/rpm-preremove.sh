#!/bin/bash
# DropSentinel RPM Pre-removal Script
...
log() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [RPM-PREREMOVE] $1" | tee -a "$LOG_FILE"
}
...
log_warning() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [RPM-PREREMOVE] ⚠️  $1" | tee -a "$LOG_FILE"
}
...
log "Starting DropSentinel RPM pre-removal process..."
...
log_success "RPM pre-removal process completed"
exit 0
